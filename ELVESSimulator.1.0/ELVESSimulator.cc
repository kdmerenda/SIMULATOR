...
ELVESSimulator::Run(evt::Event& event)
{
  ...
  if(fStatus == eGeneratePhotons){
	...
	if (eyeId == 1){
	  ...
	  for (int iTime = 0; iTime < fNPhotonsToCreate; iTime ++) ELVESData[iTime].time = ELVESData[iTime].timeEye1;
	}
	if (eyeId == 2){
	  sort(ELVESData.begin(), ELVESData.end(), by_timeEye2());
	  for (int iTime = 0; iTime < fNPhotonsToCreate; iTime ++) ELVESData[iTime].time = ELVESData[iTime].timeEye2;
	}
	if (eyeId == 3){
	  sort(ELVESData.begin(), ELVESData.end(), by_timeEye3());
	  for (int iTime = 0; iTime < fNPhotonsToCreate; iTime ++) ELVESData[iTime].time = ELVESData[iTime].timeEye3;
	}
	if (eyeId == 4){
	  sort(ELVESData.begin(), ELVESData.end(), by_timeEye4());
	  for (int iTime = 0; iTime < fNPhotonsToCreate; iTime ++) ELVESData[iTime].time = ELVESData[iTime].timeEye4;
	}
	...
	for(int iCell=0; iCell<fNPhotonsToCreate;iCell+=fPhotonDiscr){
	  ...
	  for(int iDiaPoint=0; iDiaPoint<fNumDiaGridPoints;iDiaPoint++){
	    ...
	    double photonTime=(ELVESData[iCell].time - (fLoop-1)*0.0001)*1e9;//need floop to reset time at each event to 0. Also need nanoseconds?
	    ...
	    const double weight = 2*ELVESData[iCell].ndensitynormalized;//TBD	  
	    ...
	    //    hTelCS->Fill(nIn.GetPhi(telCS)/deg,nIn.GetTheta(telCS)/deg,weight);
